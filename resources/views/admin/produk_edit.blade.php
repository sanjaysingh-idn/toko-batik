                ...
                <div class="d-flex align-items-start align-items-sm-center gap-4">
                    <div class="button-wrapper">
                        <form action="{{ route('produk.update', $produk->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <img class="img-thumbnail shadow mb-5" src="{{ asset('storage/'. $produk->image) }}" width="200" alt="Foto {{ $produk->name }}" class="img-thumbnail">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="mb-3 col-md-6">
                        ...
                        <input class="form-control @error('name') is-invalid @enderror" type="text" id="name" name="name" value="{{ $produk->name }}" />
                        @error('name')
                    ...
                    <div class="mb-3 col-md-6">
                        ...
                        <select id="id_kategori" class="select2 form-select @error('id_kategori') is-invalid @enderror" name="id_kategori" required>
                            ...
                            <option value="{{ $k->id }}" class="text-capitalize" {{ $k->id == $produk->id_kategori ? 'selected' : '' }}>{{ $k->name }}</option>
                            @endforeach
                        </select>
                        @error('id_kategori')
                        <div class="invalid-feedback">
                    ...
                    <div class="mb-3 col-md-6">
                        ...
                        <div class="input-group mt-2">
                            ...
                            <input type="number" class="form-control" name="harga" min="1" value="{{ $produk->harga }}">
                        </div>
                    </div>
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <div class="btn-group" role="group" aria-label="Basic checkbox toggle button group">
                            @foreach ($ukuran as $u)
                            <input type="checkbox" class="btn-check" id="{{ $u }}" name="ukuran[]" value="{{ $u }}" {{ in_array($u, explode(',', $produk->ukuran)) ? 'checked' : '' }}>
